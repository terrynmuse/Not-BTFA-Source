...
{
	public class StarshineGreen : ModProjectile
	{
			...
			for (int index2 = 0; index2 < Main.npc.Length; ++index2)
            {
				...
				if ((npc.active && !npc.friendly && (npc.damage > 0 && !npc.dontTakeDamage) && (double) Vector2.Distance(player.Center, npc.Center) <= (double) 300f))
					...
					if (Main.rand.Next(3) == 0)
					{
						...
						dust = Dust.NewDust(npc.Center, 0, 0, 89, newVect.X, newVect.Y);
						int dust2 = Dust.NewDust(npc.Center, 0, 0, 89, newVect2.X, newVect2.Y);
						int dust3 = Dust.NewDust(npc.Center, 0, 0, 89, newVect3.X, newVect3.Y);//74
						int dust4 = Dust.NewDust(npc.Center, 0, 0, 89, newVect4.X, newVect4.Y);
						int dust5 = Dust.NewDust(npc.Center, 0, 0, 89, newVect5.X, newVect5.Y);
						int dust6 = Dust.NewDust(npc.Center, 0, 0, 89, newVect6.X, newVect6.Y);
						int dust7 = Dust.NewDust(npc.Center, 0, 0, 89, newVect7.X, newVect7.Y);
						int dust8 = Dust.NewDust(npc.Center, 0, 0, 89, newVect8.X, newVect8.Y);
